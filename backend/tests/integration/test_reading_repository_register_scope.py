...
def test_get_ocr_metadata_returns_empty_candidates_for_non_numeric_payload(caplog) -> None:
    session = _session()
    _meter_point_id, register_ht, _register_nt, _building_id = _seed_meter_with_two_registers(session)
    reading_id = str(uuid4())
    session.add(
        ReadingRecord(
            id=reading_id,
            meter_register_id=str(register_ht),
            measured_at=datetime(2025, 1, 3, tzinfo=timezone.utc),
            value=Decimal("123"),
            source="photo",
            image_path="/uploads/meter.jpg",
            ocr_confidence=Decimal("0.8700"),
            ocr_text="123",
            ocr_candidates='[{"value": "abc", "confidence": 0.2}]',
            ocr_status="suggested",
        )
    )
    session.commit()
